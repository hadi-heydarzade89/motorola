...
class WoocommerceIR_SMS_Settings {
	...
	public function adminMenu() {
		...
		add_submenu_page( $slug, 'پیامک ووکامرس', 'پیامک ووکامرس',
	...
	public function settingPage() {
		...
		echo '<img class="logo" src="' . plugins_url( 'assets/images/persian-woocommerce-sms-logo.png', __DIR__ ) . '"/>
		<a href="https://wordpress.org/plugins/persian-woocommerce-sms" target="_blank" class="button button-secondary float-left-buttons">نسخه '.PWOOSMS_VERSION.'</a>
		&nbsp;
		...
		echo '</div>';
		echo '<div class="right-side-pwoo">';

		if ( is_plugin_inactive( 'persian-woocommerce/woocommerce-persian.php' ) ) {
			echo '<div class="notice notice-success below-h2">
                <p><img class="نصب شده" src="' . plugins_url( 'assets/images/false.png', __DIR__ ) . '"/> برای کارکرد بهتر افزونه پیامک ، و افزوده شدن امکانات بومی مانند شهرها ، اعداد فارسی و... به ووکامرس پیشنهاد می‌کنیم افزونه "ووکامرس فارسی" را نصب نمایید.
		...
		if ( is_plugin_inactive( 'persian-woocommerce-shipping/woocommerce-shipping.php' ) ) {
			echo '<div class="notice notice-error below-h2">
                <p><img class="نصب شده" src="' . plugins_url( 'assets/images/false.png', __DIR__ ) . '"/> برای محاسبه خودکار هزینه های حمل و نقل پست پیشتاز و سفارشی و پیک موتوری افزونه "حمل و نقل ووکامرس" را نصب نمایید.
	...
	<p>کد تخفیف: <code>woohost</code></p>
		...
		if ( is_plugin_inactive( 'nabik-review-reminder/nabik-review-reminder.php' ) ) {
			...
			<p>' . sprintf( 'جهت افزایش تعداد نظرات فروشگاه‌تان، می‌توانید با استفاده از <b><a href="%s" target="_blank">افزونه نظرسنجی خودکار ندا</a></b> با ارسال خودکار پیامک و ایمیل، برای هر سفارش از مشتریان خود درخواست ثبت نظر کنید.', 'https://yun.ir/pwsmsneda?utm_source=sidebar' ) . '</p>
	...
	public function footerVersion( $text ) {
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'persian-woocommerce-sms-pro' ) {
			$text = 'پیامک ووکامرس نگارش ' . PWOOSMS_VERSION;
		}
	...
	public function pwoo_sms_array_sanitize_text_field( $array ) {
		foreach ( $array as $key => &$value ) {
			if ( is_array( $value ) ) {
				$value = $this->pwoo_sms_array_sanitize_text_field( $value );
			} else {
