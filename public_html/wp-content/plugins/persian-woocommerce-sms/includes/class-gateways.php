...
class WoocommerceIR_SMS_Gateways {
	...
	public function limosms() {
		...
		try {
			...
			$remote = wp_remote_post( 'https://api.limosms.com/api/sendsms',$args );
	...
	public function ippanel( $datas ) {
	    if(!class_exists("pwoo_sms_ippanel")){
	        require_once WP_PLUGIN_DIR . "/persian-woocommerce-sms-ippanel/pwoo_sms_ippanel.php";
	    }
		if(class_exists("pwoo_sms_ippanel")){
		$ippanel = new pwoo_sms_ippanel;
		return $ippanel->SendSMS( $datas );
	}else{
		return "افزونه مکمل ippanel غیرفعال است. لطفا از صفحه افزونه ها فعال نمایید.";
	}
}
}