...
jQuery(function ($) {
    ...
    $('#license_key').on('input', function () {
        $('#apikey-container').html(
            '<p style="color: red; margin-top: 15px;">Please, save the plugin settings.</p>'
        );
    });
    ...
    $(document).on('click', '.web357-activate-api-key-btn', function (e) {
        ...
        $.ajax({
            ...
            data: {
                action: 'web357_license_key_validation',
                key: key,
                domain: domain,
                nonce: nonce,
            },
            ...
            success: function (response) {
                ...
                if (response.type == 'success') {
                    ...
                    alert(
                        'There is a problem. Your key could not be validated. Please, contact us at support@web357.com'
                    );
                }
            },
...
if (typeof angular !== 'undefined') {
    // angular is loaded successfully
    angular.module('usinApp').run([
        '$templateCache',
        function ($templateCache) {
            'use strict';

            $templateCache.put(
                'views/user-list/profile-editable-field.html',
