...
final class WOOF {
    ...
    public function init() {
        ...
        if ($first_init != 1) {
            ...
            update_option('woof_set_automatically', 0); //disabled on 13-07-2023
    ...
    public function enqueue_scripts_styles() {
        ...
        if ($icheck_skin != 'none') {
            ...
            if ($icheck_skin != 'none') {
                ...
                //wp_enqueue_style('icheck-jquery-color', WOOF_LINK . 'js/icheck/skins/' . $icheck_skin[0] . '/' . $icheck_skin[1] . '.css', [], WOOF_VERSION);
                wp_enqueue_style('icheck-jquery-color-flat', WOOF_LINK . 'js/icheck/skins/flat/_all.css', [], WOOF_VERSION);
                wp_enqueue_style('icheck-jquery-color-square', WOOF_LINK . 'js/icheck/skins/square/_all.css', [], WOOF_VERSION);
                wp_enqueue_style('icheck-jquery-color-minimal', WOOF_LINK . 'js/icheck/skins/minimal/_all.css', [], WOOF_VERSION);
            }
        }
        ...
        if (!empty(WOOF_EXT::$includes['css'])) {
            foreach (WOOF_EXT::$includes['css'] as $css_key => $css_link) {
                wp_enqueue_style($css_key, $css_link, [], WOOF_VERSION);
            }
        }


        wp_enqueue_style('woof-switcher23', WOOF_LINK . 'css/switcher.css', [], WOOF_VERSION);
    }
...
//
    ...
    public function wp_load_js() {
        ...
        <?php
        echo apply_filters('woof_wp_load_js', '') . ';'; //vars from another exts
    ...
    public function dynamic_count($curr_term, $type, $additional_taxes = '', $meta_term = array(), $custom_type = "") {
        ...
        if (isset($_GET['min_price']) && isset($_GET['max_price'])) {
            $old_price['min_price'] = $_GET['min_price'];
            $old_price['max_price'] = $_GET['max_price'];
        }
        ...
        if ($this->is_isset_in_request_data('min_price') AND $this->is_isset_in_request_data('max_price')) {
            ...
            //fix for radio price + seourl
            if (!empty($old_price) && ($old_price['min_price'] != $request['min_price'] || $old_price['max_price'] != $request['max_price'])) {
                foreach ($args['meta_query'] as $key => $meta_data) {
                    if (is_array($meta_data)) {
                        foreach ($meta_data as $meta_key => $meta_data) {

                            if (isset($meta_data['key']) && '_price' == $meta_data['key']) {
                                if ($meta_data["compare"] == '<=') {
                                    $args['meta_query'][$key][$meta_key]["value"] = $old_price['max_price'];
                                } elseif ($meta_data["compare"] == '>=') {
                                    $args['meta_query'][$key][$meta_key]["value"] = $old_price['min_price'];
                                }
                            }
                        }
                    }
        ...
        $args['woof_query'] = 'product_count'; //key to find this query

        $query = new WP_QueryWoofCounter(apply_filters('woof_dynamic_count_attr', $args, $custom_type));
    ...
    public function is_woof_use_chosen() {
        ...
        return apply_filters('woof_use_chosen', $is);
    }
    ...
    public function get_tax_query($additional_taxes = '') {
        ...
        return apply_filters('woof_get_tax_query', $this->parse_tax_query($res));
    }
    ...
    public function get_meta_query($args = array()) {
        ...
        $meta_query = array_merge(array('relation' => 'AND'), WC()->query->get_meta_query());
        ...
        // end fix
        // For meta filter
        return apply_filters('woof_get_meta_query', $meta_query);
    }
