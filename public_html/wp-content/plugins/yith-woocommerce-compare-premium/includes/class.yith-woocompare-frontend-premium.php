...
if ( ! class_exists( 'YITH_Woocompare_Frontend_Premium' ) ) {
	...
	class YITH_Woocompare_Frontend_Premium extends YITH_Woocompare_Frontend {
		...
		public function premium_localize_args( $args ) {
			return array_merge(
				...
				array(
					...
					'filter_nonce'     => wp_create_nonce( 'yith_woocompare_filter_action' ),
		...
		public function yith_woocompare_filter_by_cat_ajax() {
			if ( ! isset( $_REQUEST['yith_compare_cat'] ) || ! isset( $_REQUEST['security'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['security'] ) ), 'yith_woocompare_filter_action' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				die();
			}
