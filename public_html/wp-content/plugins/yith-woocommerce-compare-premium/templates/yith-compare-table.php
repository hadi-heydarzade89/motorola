...
<div id="yith-woocompare" class="woocommerce <?php echo $fixed ? esc_attr( 'fixed-compare-table' ) : ''; ?>">
	...
	if ( empty( $products ) ) :
		 ...
		 * @return string
		 */
		echo '<p>' . wp_kses_post( apply_filters( 'yith_woocompare_empty_compare_message', __( 'No products added in the comparison table.', 'yith-woocommerce-compare' ) ) ) . '</p>';
	else :
		/**
									...
									if ( $product instanceof WC_Product_Variation && apply_filters( 'yith_woocompare_support_show_single_variations', true ) ) {
										...
										if ( strpos( $field, 'pa_' ) !== false ) {
											if ( isset( $attributes[ $field ] ) && '' !== $attributes[ $field ] ) {
												echo wp_kses_post( ucwords( $attributes[ $field ] ) );
											} elseif ( is_array($parent_attributes) & ! empty( $parent_attributes ) ) {
