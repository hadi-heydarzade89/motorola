...
(function ($) {
	...
	if ( typeof yithUpgradePlugins === 'undefined' ) {
		return;
	}
	...
	$(document).on('click', 'a.yith-update-link', function (event) {
		...
		$.ajax({
			type: 'POST',
			url: yithUpgradePlugins.ajaxUrl,
			data: {
				'action': "update-plugin",
				'plugin': t.data('plugin'),
				'slug': t.data('slug'),
				'name': t.data('name'),
				'_ajax_nonce': yithUpgradePlugins.security
			},
			beforeSend: function () {
				p_wrapper.text(yithUpgradePlugins.l10n.updating.replace('%s', t.data('name')));
			...
			success: function (response) {
				...
				if ( response.success === true ) {
					...
					result_text = yithUpgradePlugins.l10n.updated;
				} else {
					notice_wrapper.addClass('notice-error');
					result_text = yithUpgradePlugins.l10n.failed;
				}
