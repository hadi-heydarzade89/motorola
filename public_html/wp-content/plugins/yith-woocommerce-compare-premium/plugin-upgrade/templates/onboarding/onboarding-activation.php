 ...
 * @var string $plugin_init Current processing plugin.
		...
		<form id="activation-form" method="POST" action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>">
			...
			<p class="form-row">
				<label for="licence_key">
					...
					<a href="#" id="yith-licence-where-find-these" class="small-link" tabindex="-1"><?php echo esc_html_x( 'Where to find your license key?', 'Link on activation license panel', 'yith-plugin-upgrade-fw' ); ?></a>
				</label>
				<span>
			...
			<input type="hidden" name="product_init" value="<?php echo esc_attr( $plugin_init ); ?>"/>
		</form>
...
<?php wp_print_scripts( 'yith-licence-panel' ); ?>
