...
use YITH\PluginUpgrade\Licences\YITH;
...
if ( ! class_exists( 'YITH\PluginUpgrade\Services' ) ) :
	...
	class Services {
		 ...
		 * @throw  Exception
		 */
		public function get_token( $slug, $plugin_init ) {

			$licence = $this->get_licence_info( $plugin_init );

			if ( is_null( $licence ) || ! $licence->is_activated() ) {
				throw new \Exception( __( 'Impossible proceed with token request. Licence is not active', 'yith-plugin-upgrade-fw' ) );
			}

			$tokens = $this->get_tokens();

			if ( $tokens && isset( $tokens[ $slug ] ) && $tokens[ $slug ]['exp'] > current_time('timestamp') ) {
		 ...
		 * @return array
		 */
		protected function get_auth_args( $licence ) {
		 ...
		 * @throw  Exception
		 */
		public function get_new_token( $licence ) {
			$token = array();
		 ...
		 */
		protected function get_licence_info( $plugin_init ) {
